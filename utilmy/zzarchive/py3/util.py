...
def pd_df_tojson(df:pd.DataFrame,  tname='table1', format='column'):
   ''' {'table1': {"index": [ 0, 1 ,3 ],
                 "col1" : [ "250", "1" ,"3" ] }
       }
   '''
...
# selection
def pd_selectrow(df:pd.DataFrame, **conditions):
...
def pd_find(df:pd.DataFrame, regex_pattern='*', col_restrict=[], isnumeric=False, doreturnposition=False) :
...
def pd_df_todict(df:pd.DataFrame, colkey='table', excludekey=[''], onlyfirstelt= True) :
...
def pd_insertcol(df:pd.DataFrame, colname, vec):
