...
class TimeSeries(Dataset):
    ...
    def _sample_ts(pandas_df, desired_len):
        ...
        pandas_df: input pandas df with 'target' columns e features
        desired_len: desired sample length (number of rows)
        padding_val: default is 0
        initial_obs: how many observations to skip at the beginning
    ...
    def next_batch(self, batch_size, n_steps,
                   ...
                   padding_value=0):
        """
        batch_size: how many time series to be sampled in this batch (int)
        n_steps: how many RNN cells (int)
        target_var: (str)
        verbose: (boolean)
        padding_value: (float)
